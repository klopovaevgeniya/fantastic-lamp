...
def readLine(x,y,inpLen=10,oldStr=""):
    ...
    while True:
        ...
        if ord(ch)==27:   # ESC
            ...
            break
        elif (ch.isalpha() or (ch in (" ","-","_"))) and len(res)<inpLen:
            res += ch
...
sgFileName = "textGame.txt"
curPlayer  = {"name":"", "energy":100, "roomNum":0, "matches":5,
...
rooms = [
         ...
         },
         {
            "num"        : 7,
            "name"       : "Винный погреб",
            "description": "Вы добрались до винного погреба...Здесь было много бутылок различных напитков из разных стран."+
                           "На полу тоже лежали бутылки..некоторые пустые, какие-то были разбиты. От разлитого вина остались следы."+
                           "которые привели вас к выходу из этого страшного дома...",
            "visited"    : 0,
            "medicals"   : 0,
            "matches"    : 0,
            "knifes"     : 0,
...
def setPlayer():
    printColoredText(3, 8, "Введите имя игрока: ", "white", "blue")
    curPlayer["name"]     = readLine(23,8,37,curPlayer["name"])
...
def saveGame(fileName):
    sgFile = open(fileName, "w+")
    sgData =[curPlayer, rooms]
...
def startGame():
    ...
    if os.path.exists(sgFileName):
        printColoredText(3, 2, "Обнаружен файл с сохраненной игрой: "+sgFileName, "white", "blue")
        printColoredText(3, 3, "Хотите загрузить сохраненную игру? (Да: 1 / Нет: 2)", "white", "blue")
        printColoredText(3, 4, "Ваш выбор ------> ", "white", "blue")
        ...
        if choice=='1':
            if loadGame(sgFileName):
                isPlayerNeed = False
            else: printColoredText(3, 6, "Ошибка загрузки сохраненной игры!", "red", "white")
